...
inline01.coef %>%
  ...
  geom_smooth(method = "rlm", se = F,
              ...
              alpha = 0.2, 
              size = 0.5) +
  geom_point(size = 2.75, shape = 21, fill = "steelblue3") +
...
# dotplot with errorbars
ggplot(rr02.coef, aes(reorder(NAME, abs(.resid)), weighted.fit))  +
  geom_point(aes(reorder(NAME, abs(.resid)), perCapitaFFL),
             shape = 1, 
             size = 4) +
  geom_point(aes(reorder(NAME, abs(.resid)), perCapitaFFL),
             shape = 20, 
