...
prp(edu.rpart01, 
    type = 1, extra = 1, digits = 4, under = F,
    branch.lty = 1, branch.lwd = 1, 
    cex = 1,  split.cex = 1.1, nn.cex = 1,
    box.palette = "BuBn",
    family = "GillSans")
...
# rpart ggdendro 
rp01 <- dendro_data(edu.rpart01, type = "triangle")
str(rp01)

ggplot(segment(rp01)) + 
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend)) + 
  geom_text(aes(x = x, y = y, label = label), 
            size = 4, vjust = 1.75, hjust = 1.05,
            data = label(rp01)) +
  geom_text(data = rp01$leaf_labels, 
